...
app = get_modal_app("browser_control_using_grpo_qwen")
image = get_docker_image()
hf_models_volume = get_volume("hf-model-cache")
model_checkpoints_volume = get_volume("browser_control_using_grpo_qwen")

# TODO : making sure that parameters to rollout func are matching GRPOTrainer API
...
def rollout_func(
    ...
    for i,prompt_text in enumerate(prompts):
        print(f"\n[DEBUG] Processing prompt {i+1}/{len(prompts)}")
...
def rollout_once(
    ...
    for step_num in range(max_steps):
        ...
        # Create prompt from observation tree (text-only using accessibility tree)
        ...
        # Reward shaping : Success is important bro
        if result.done and step_reward > 0:
            ...
            completion_rewards.append(0.0) # Failed 👎
...
def make_user_prompt(goal: str,step_num: int,axtree: str,error: str = "") -> str:
    """Creating user prompt from observation"""
    ...
    # Including the accessibility tree(truncated for context)
...
def parse_action(response_txt : str) -> str:
    '''Parsing BrowserGymAction from model response'''
    #Extract first line that looks like an action
    for line in response_txt.strip().split("\n"):
        line = line.strip()
...
def reward_completion(completions : list[str],**kwargs) -> list[float]:
    """Reward if you complete a task"""
    rewards = kwargs.get("completion_reward") if kwargs else None
    if rewards is None:
        return [0.0 for _ in completions]
    return [float(r) for r in rewards]

def create_peft_config(config:FineTuneConfig) -> LoraConfig | None:
    
    """
    Create LoRA configuration from FineTuneConfig
...
@app.function(
    ...
    secrets = get_secrets(),
    timeout = 2*60*60, # 2 hours enough bro
    retries = get_retries(max_retries = 1),
    max_inputs = 1,
)

def fine_tune(config : FineTuneConfig) -> None:
    '''
    Fine tunes a language model using the BrowserGym environment and the GRPO algorithm
    '''
    ...
    # Path where we save intermediate and final model checkpoints
    output_dir = get_path_model_checkpoints(config.wandb_experiment_name)

    print("Here we are creating GRPOConfig ..")
    grpo_config = GRPOConfig(
        ...
        generation_batch_size=config.generations_batch_size,
    ...
    trainer_stats = trainer.train() # Takes max 2 hours because i kept cutoff time
...
def main(config_file_name: str):
    config = FineTuneConfig.from_yaml(file_name = config_file_name)

    try:
        fine_tune.remote(config=config)
        print("Fine-tuning job completed successfully !!")
    except Exception as e:
        print(f"failed fine-tuning job : {e}")
        raise e
